...
# -------------------- 설정 --------------------
OUTPUT_DIR = "outputs"
LOGIC_CSV = "26SS_logic_final.csv"            # 26개 코드/색 CSV (없어도 동작)
THREADBOOK_CSV = "imjae_threadbook_full.csv"  # 실북 CSV (없어도 동작)
...
def load_26ss_logic(path: str) -> Dict:
    ...
    with open(path, "r", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            code = (rlow.get("code") or "").strip()
            if not code: continue

            if all(k in rlow for k in ["h_r","h_g","h_b"]) and rlow["h_r"] and rlow["h_g"] and rlow["h_b"]:
                h_rgb = (int(float(rlow["h_r"])), int(float(rlow["h_g"])), int(float(rlow["h_b"])))
            elif rlow.get("h_hex"):
                h_rgb = hex_rgb(rlow["h_hex"])
            else:
                continue
            h_map[code] = h_rgb
            codes.append(code)
            ...
            # fabric 스와치(자동 선택용)
            if all(k in rlow for k in ["fabric_r","fabric_g","fabric_b"]) and rlow["fabric_r"] and rlow["fabric_g"] and rlow["fabric_b"]:
                swatch_map[code] = (int(float(rlow["fabric_r"])), int(float(rlow["fabric_g"])), int(float(rlow["fabric_b"])))
            elif rlow.get("fabric_hex"):
                swatch_map[code] = hex_rgb(rlow["fabric_hex"])
    return {"codes": sorted(set(codes)), "h_map": h_map, "swatch_map": swatch_map}

# -------------------- 실북 Top4 (csv 모듈로 읽음; 선택) --------------------
def load_threadbook(path: str) -> Optional[List[Dict]]:
    ...
    try:
        ...
        with open(path, "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                rlow = {k.lower(): v for k, v in row.items()}
                if not all(k in rlow for k in ["thread_no","thread_name","r","g","b"]):
                    # 다른 헤더 표기 대응
                    if not all(k in rlow for k in ["thread_no","thread_name","r","g","b"]):
                        continue
                rows.append({
                    "Thread_No": (rlow.get("thread_no") or row.get("Thread_No") or "").strip(),
                    "Thread_Name": (rlow.get("thread_name") or row.get("Thread_Name") or "").strip(),
                    "R": int(float(rlow.get("r") or row.get("R") or 0)),
                    "G": int(float(rlow.get("g") or row.get("G") or 0)),
                    "B": int(float(rlow.get("b") or row.get("B") or 0)),
                })
...
# -------------------- FastAPI 앱 --------------------
app = FastAPI(title="HAZZYS Embroidery Recommender API", version="1.0.0")  # ←←← 중요: app 변수 존재
...
def _pick_code_auto(fabric_rgb: Tuple[int,int,int], codes: List[str], swatch_map: Dict[str,Tuple[int,int,int]]) -> str:
    if not swatch_map:
        return codes[0] if codes else "26SS_09"
    # fabric와 가장 가까운 스와치 코드 선택
    best, bestc = None, None
    for c in codes:
        if c in swatch_map:
            d = euclid(fabric_rgb, swatch_map[c])
            if best is None or d < best: best, bestc = d, c
    return bestc or (codes[0] if codes else "26SS_09")
...
async def render_endpoint(
    ...
    if not codes:
        raise HTTPException(500, "No 26SS codes available (CSV missing).")
    ...
    if pick == "auto" or pick not in h_map:
        pick = _pick_code_auto(fabric_rgb, codes, swatch_map)
    h_rgb = h_map[pick]
